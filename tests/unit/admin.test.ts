...
describe('P0-8 admin service', () => {
  ...
  it('上下架切换即时生效', async () => {
    ...
    expect((await db.query.products.findFirst({ where: eq(products.id, product.id) }))?.status).toBe('off');
    await setProductStatus(product.id, 'on');
    expect((await db.query.products.findFirst({ where: eq(products.id, product.id) }))?.status).toBe('on');
  ...
  it('SKU 价格/原价/库存更新；更新写 admin_adjust 日志并同步 min_price', async () => {
    ...
    const { products } = await import('@/server/db/schema');
    const { eq } = await import('drizzle-orm');
    expect((await db.query.products.findFirst({ where: eq(products.id, product.id) }))?.minPrice).toBe(5500);
    // 库存不变不写日志
  ...
  it('库存不得小于 locked_stock；非法价格拒绝', async () => {
    ...
    const { skus: s } = await createProductWithSkus({}, [{ spec: { 颜色: '黑' }, price: 5000, stock: 10 }]);
    await placeOrder(u.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 3 }, addressId: addr.id });
    await expect(updateSku(s[0]!.id, { price: 5000, originalPrice: null, stock: 2 })).rejects.toThrowError(/锁定|不能小于/);
    await updateSku(s[0]!.id, { price: 5000, originalPrice: null, stock: 3 }); // 恰好等于 locked 允许
    expect((await getSku(s[0]!.id)).stock).toBe(3);
    await expect(updateSku(s[0]!.id, { price: -1, originalPrice: null, stock: 5 })).rejects.toThrowError(/价格/);
    await expect(updateSku(999999, { price: 100, originalPrice: null, stock: 1 })).rejects.toThrowError(/不存在/);
  });
  ...
  it('发货仅 PAID（复用 order.shipOrder，已在 order.query 测试覆盖状态约束）', async () => {
    ...
    const addr = await createAddress(u.id, { isDefault: true });
    const { skus: s } = await createProductWithSkus({}, [{ spec: { 颜色: '黑' }, price: 5000, stock: 10 }]);
    const o = await placeOrder(u.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 1 }, addressId: addr.id });
    await expect(shipOrder(o.orderNo, { carrier: 'X', trackingNo: '1' })).rejects.toThrowError(/发货/);
  ...
  it('统计卡片：今日订单数 / 已支付 GMV / 待发货数 / 售罄 SKU 数', async () => {
    ...
    const o1 = await placeOrder(u.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 1 }, addressId: addr.id });
    const o2 = await placeOrder(u.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 2 }, addressId: addr.id });
    ...
    // 一笔昨日订单（不计今日）
    const o3 = await placeOrder(u.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 1 }, addressId: addr.id });
    const { orders } = await import('@/server/db/schema');
    const { eq } = await import('drizzle-orm');
    await db.update(orders).set({ createdAt: new Date(Date.now() - 2 * 86400_000) }).where(eq(orders.id, o3.id));
  ...
  it('商品列表分页/搜索；订单列表状态筛选（含全部用户订单）', async () => {
    ...
    const o1 = await placeOrder(u1.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 1 }, addressId: a1.id });
    await placeOrder(u2.id, { source: { type: 'buyNow', skuId: s[0]!.id, quantity: 1 }, addressId: a2.id });
